...
論理ボリュームをXFSファイルシステムでフォーマットし、データの読み書きができる狀態にします。
...
論理ボリュームを /mnt/lvmtest にマウントします。

---
# 📅 2026-03-26
---

## `sudo lvextend -L +200M /dev/vg_test/lv_data`
lv_dataの論理ボリュームを200MB拡張します。

---

## `sudo xfs_growfs /mnt/lvmtest`
XFSファイルシステムを拡張後のボリュームサイズに合わせて拡大します。

---

## `df -h | grep lvmtest`
lvmtestが700MBに拡張されているか確認します。

---

## `sudo blkid /dev/vg_test/lv_data`
lv_dataのUUIDを確認します。fstabへの記載に使用します。

---

## `sudo vi /etc/fstab`
lv_dataの永続マウント設定を追記します。
